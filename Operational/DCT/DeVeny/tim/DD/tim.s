...
;    board set.  This one is for the DeVeny e2v CCD42-10
;
...
TIMCAPABLE	EQU	FINDCAPABLE+SNGLCAPABLE+STRPCAPABLE+FDOTCAPABLE+SDOTCAPABLE

	ORG	P:,P:

; Remove SHUTTER_CC advertisement as per Confluence July 4 #3
;CC	EQU	ARC22+ARC47+SHUTTER_CC+SPLIT_SERIAL+SUBARRAY+BINNING
CC	EQU	ARC22+ARC47+SUBARRAY+BINNING+SHUTTER_CC
...
; CCD42-10 isn't frame transfer so comment out the part about that.
...
;	MOVE	#<(END_SERIAL_SKIP_LEFT-SERIAL_SKIP_LEFT-1),M1 ; Modularity
	MOVE	#<(END_SERIAL_SKIP_WAVE-SERIAL_SKIP_WAVE-1),M1 ; Modularity
...
;NS_CLR	DC      280 		; To clear serial register, twice
        IF      @SCP("COLS_SETTING","2098")
NS_CLR	DC      2100 		; To clear serial register, twice
	ENDIF
        IF      @SCP("ROWS_SETTING","515")
NPCLR   DC      1030	    ; To clear parallel register, twice 
	ENDIF
...
SH_DEL	DC	10		; Delay in milliseconds between shutter closing 
	...
	ENDIF
        IF      @SCP("SHDEL_SETTING","100")
SH_DEL	DC	100		; Delay in milliseconds between shutter closing 
				;   and image readout.  Actual delay is 1.5x
				;   as long as advertised, e.g. 4.5 ms for 3.
	ENDIF
CONFIG	DC	CC		; Controller configuration
; Readout peculiarity parameters
SERIAL_SKIP 	DC		SERIAL_SKIP_WAVE 	   ; Serial skipping waveforms
SERWAVLEN	DC	5			; Serial read waveform table length

SERIAL_CLOCK    DC      SERIAL_CLOCK_WAVE      ; Serial waveform table
INITIAL_CLOCK   DC      INITIAL_CLOCK_WAVE     ; Serial waveform table
IS_PAR_CLK      DC      PARALLEL               ; Addr. of ganged parallel clocking
S_PAR_CLK       DC      PARALLEL               ; Addr. of storage parallel clocking
IS_PAR_CLR      DC      PARALLEL_CLEAR         ; Addr. of ganged parallel clear
S_PAR_CLR       DC      PARALLEL_CLEAR         ; Addr. of storage parallel clear
...
S_SIZE		DC      1000	; storage area =  half of total rows.  Not used for DeVeny
...
; Include the waveform table
	INCLUDE "deveny.waveforms.s" ; Readout and clocking waveforms
