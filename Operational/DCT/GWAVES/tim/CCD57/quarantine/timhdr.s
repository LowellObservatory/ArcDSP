...
; NOTE
; NOTE
; the SPLIT_P, and MPP X:<STATUS bits are pre-empted.
; the SHUT X:<STATUS bit, and all other >= 8 are not used
; assumption is that a GWAVES class camera will never use a shutter.
ST_ABRT EQU     6       ; Set if an abort (readout or exp) in progress
