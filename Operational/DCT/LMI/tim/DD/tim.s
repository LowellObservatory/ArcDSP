...
TIMCAPABLE	EQU	FINDCAPABLE+SNGLCAPABLE+STRPCAPABLE+BASCCAPABLE+FDOTCAPABLE+SDOTCAPABLE

	ORG	P:,P:

; Remove SHUTTER_CC advertisement as per Confluence July 4 #3
;CC	EQU	ARC22+ARC47+SHUTTER_CC+SPLIT_SERIAL+SUBARRAY+BINNING
CC	EQU	ARC22+ARC47+SPLIT_SERIAL+SPLIT_PARALLEL+SUBARRAY+BINNING+SHUTTER_CC
...
; Do the frame transfer if STORAGE==0, else skip it.
	MOVE	Y:<S_SIZE,X1
	JSR	<CLOCK_WAIT
; mitigation attempt 2- remove ishift
	JSR	ISHIFT		; Do the frame transfer
;	JSR	<CLOCK_WAIT  ; mitigation attempt 1
...
; If #NO_SKIP == 0 skip rows up to first subframe, storage clocks only
	...
	MOVE    Y:<IS_PAR_CLR,R0        ; SR kept clear with DG
	JSET    #STORAGE,X:ISTATUS,GANG_SK  ; if STORAGE == 0 store clocks only
;        mitigation attempt 3- don't use S_PAR_CLR (but it was a full fr test)
	MOVE    Y:<S_PAR_CLR,R0		; SR kept clear with DG
GANG_SK
	JSR     <CLOCK
	NOP
L_SKIP1
...
; NOT DONE- check for ABEXP in expose only
...
; Abort the readout currently underway (cmd destination)
ABR_RDC
	CLR	A
	MOVE	Y:<TESTLOC1,A
	ADD	#1,A
...
CONTINUE_RD	DO      Y:<NPBIN,LPR_I  ; Transfer # of rows, with binning
        MOVE    Y:<IS_PAR_CLK,R0
	JSET	#STORAGE,X:ISTATUS,GANGED  ; if STORAGE == 1 ganged parallels
;       mitigation attempt 4- don't use S_PAR_CLK- seemed to work
	MOVE    Y:<S_PAR_CLK,R0		; if STORAGE == 0 store clocks only
...
;	MOVE	#<(END_SERIAL_SKIP_LEFT-SERIAL_SKIP_LEFT-1),M1 ; Modularity
	MOVE	#<(END_SERIAL_SKIP_SPLIT-SERIAL_SKIP_SPLIT-1),M1 ; Modularity
...
L_READ  CLR	A
	...
	MOVE	#>5,A0			; If binning > 4, do general binning
...
; This is the routine for serial binning from 1 to 4 using hardwired waveforms
...
DACLOGPWR	EQU	$100	; for the poweron dac log
DACLOGBASE	EQU	$200	; for the operational dac log
DACLOGLAST	EQU	$500	; for the operational dac log
DACLOGFILL      DC	0
	ENDIF
...
;NS_CLR	DC      280 		; To clear serial register, twice
        IF      @SCP("COLS_SETTING","2000")
NS_CLR	DC      2100 		; To clear serial register, twice
	ENDIF
        IF      @SCP("COLS_SETTING","6144")
NS_CLR	DC      6144 		; To clear serial register, twice
	ENDIF
        IF      @SCP("ROWS_SETTING","2000")
NPCLR   DC      4000	    	; To clear parallel register, twice 
	ENDIF
        IF      @SCP("ROWS_SETTING","6160")
NPCLR   DC      12320	    	; To clear parallel register, twice 
	ENDIF
...
; TST_DAT	DC	0		; Temporary definition for test images
        IF      @SCP("SHDEL_SETTING","500")
SH_DEL	DC	500		; Delay in milliseconds between shutter closing 
				;   and image readout.  Actual delay is 1.5x
				;   as long as advertised, e.g. 4.5 ms for 3.
	ENDIF
        IF      @SCP("SHDEL_SETTING","1500")
SH_DEL	DC	1500		; tbida empirical for lmi, 2012 Oct 26 report
	ENDIF
...
SERIAL_CLOCK    DC      SERIAL_CLOCK_SPLIT      ; Serial waveform table
INITIAL_CLOCK   DC      INITIAL_CLOCK_SPLIT     ; Serial waveform table
IS_PAR_CLK      DC      ABCD_DOWN               ; Addr. of ganged parallel clocking
S_PAR_CLK       DC      AB_DOWN                 ; Addr. of storage parallel clocking
IS_PAR_CLR      DC      ABCD_CLEAR_DOWN         ; Addr. of ganged parallel clear
S_PAR_CLR       DC      AB_CLEAR_DOWN           ; Addr. of storage parallel clear
...
IFRAMES         DC      1       ; Number of Series frames to Loop over
        IF      @SCP("ROWS_SETTING","2000")
S_SIZE   DC      1000	    	; storage area =  half of total rows
	ENDIF
        IF      @SCP("ROWS_SETTING","6160")
S_SIZE   DC      3080	    	; storage area =  half of total rows
	ENDIF
;S_SIZE		DC	1000	; Number of rows in the Storage Array
...
; Include the waveform table
	INCLUDE "lmi.waveforms.s" ; Readout and clocking waveforms
