...
; open (X0 = 0) or close (X0=10) shutter, h/w only.
SET_SHUTTER_STATE
	MOVE	X:LATCH,A
	AND	#$FFEF,A
	OR	X0,A
...
; Open the shutter conditionally based on the shutter status bit #SHUT
; This is set by the host using dspwrm to the X:<STATUS unless the frame
; is a dark
; Open the shutter by calling SET_SHUTTER_STATE
C_OSHUT
...
; Close the shutter by calling SET_SHUTTER_STATE
...
; after closing shutter be sure to wait SH_DEL msec to let the blades close
; before shifting the CCD array for readout.
        MOVE    Y:<SH_DEL,A
        TST     A
        JLE     <S_DEL0
        MOVE    #25000,X0
...
; not currently exposing, so this is a no-op.
        JMP     <FINISH
DO_ABEXP
...
; Remove for gen-iii per June 6 #2
	...
	JMP	ERROR			; unknown info argument

; LMI does not use the thermocooler status bits
	COMMENT	*
...
PWR_ON
	...
	CLR	A
	MOVE	#DACS_INV,R0		;  standard (IDLE) INV levels here

        IF      @SCP("DACSLOG","SUPPORTED")
	MOVE	#DACLOGPWR,R7		; lower part of the log
	ENDIF

	MOVE	A,Y:<DUALDD_STAT
	JSR	<SET_BIASES		; Turn on the DC bias supplies

        IF      @SCP("DACSLOG","SUPPORTED")
; the DACLOGPOWER buffer is only written once and doesn't wrap
	MOVE	#DACLOGBASE,B1
	NOP
	MOVE 	B1,X:DACLOGFILL	; set up the operational buffer
	ENDIF

	MOVE	#IDLE,R0		; Put controller in IDLE state
	MOVE	R0,X:<IDL_ADR

;       as per Confluence July 4 2010 #5
	MOVE	#$24,R1
	MOVE	R1,X:<STATUS
;       as per Confluence July 4 2010 #4 and July 21 note
	MOVE	Y:<AMPVAL,X0		
...
; LMI does not need to set VRD2 or VRD3
	COMMENT	*
...
; This is not accessible from LOIS, so make it a no-op. If it were to be
; implemented would need an argument
;
SETBIAS	
...
;   them from the 'DACS' table provided in R0
SET_BIASES
	SER_ANA
...
;  VALIDATE X0 here (between 1 and something reasonable)
	CLR	A
	MOVE	Y:(R0),A1
	NOP
	NOP
...
SB_FAIL
	...
	NOP
	MOVE	A,Y:<DUALDD_STAT
;        BSET    #ST_SBFAIL,X:<STATUS    ; we don't ever clear this
...
; Now modified as per manual for ARC-47 (1E) video board and ARC-E2V57 
; sample code
;  Command syntax is  SGN [ #BD]  #GAIN  #SPEED, #GAIN = 0-15 (1.0-4.75)
;					  #SPEED = 1-15
; 	Correct command ptr to R3 as per "Four Points" #2
;  fixes to match gen-iii as per June 30, #15
; replaced with ARC version for E2V57- except BD is set unconditionally
; to 0 for backwards compatibility at the LOIS end.
;

ST_GAIN	
;	MOVE    X:(R3)+,A       ; Board number
;       LSL     #20,A
	CLR	A		; Video bd must be at 0.
        MOVE    #$0D0000,X0
        MOVE    A1,X1
        OR      X0,A
        MOVE    X:(R3)+,X0      ; Gain
...
;        JSR     <XMIT_A_WORD    ; Transmit A to TIM-A-STD
        ...
        MOVE    X:(R3)+,A       ; Time constant (speed)
        LSL     #4,A
        OR      X1,A1           ; Board number is in bits #23-20
        MOVE    #$0C0100,X0
        OR      X0,A
	NOP
	MOVE	A,Y:<TESTLOC1	; debug
	MOVE	A,Y:DAC_GNSPD+1
;        JSR     <XMIT_A_WORD    ; Transmit A to TIM-A-STD
        JMP     <FINISH


; traditional gain-speed code
	COMMENT	*
...
; This is fairly complex.  The first value sent is NPR*IFRAMES (NAXIS2*NAXIS3).
; The second value is NSR aka NAXIS1, but if NBOXES > 0, it is NSR*NBOXES
; this matches nasa42, but since naxis1,2 for a full frame is > 2^12
; there's a max of 1024 for IFRAMES. If we used an adaptive algorithm we
; could do better.
; Meantime LOIS will have to enforce these limits.
...
PCI_READ_IMAGE
	...
	MOVE	#'RDA',B1
	JSR	<XMT_WRD
	MOVE	Y:NPR,X0		; NPR = NAXIS2
	MOVE	Y:IFRAMES,X1		; IFRAMES = NAXIS3
	MPY	X0,X1,B
	ASR	B		; Correct for multiplication left shift
	MOVE	B0,B1	
	JSR	<XMT_WRD		; Send NPR*IFRAMES to PCI card
