...
; Set all the waveform addresses depending on which readout/binning mode
	MOVE	#'__A',A		; LEFT Amplifier = readout #0
	CMP	X0,A
	JNE	<CMP_B
ALIAS_A
	MOVE	X0,Y:<AMPVAL		; save the amp value
        ...
        MOVE    #ABCD_CLEAR_UP,Y0
        MOVE    Y0,Y:IS_PAR_CLR          ; Clear full CCD
        MOVE    #CD_CLEAR_UP,Y0
        MOVE    Y0,Y:S_PAR_CLR           ; Clear storage area only
   
	; serials for general binning
	MOVE	#SERIAL_SKIP_EH,Y0
	MOVE	Y0,Y:SERIAL_SKIP
	MOVE    #INITIAL_CLOCK_EH,Y0
	MOVE    Y0,Y:INITIAL_CLOCK
	MOVE    #SERIAL_CLOCK_EH,Y0
	MOVE    Y0,Y:SERIAL_CLOCK
        MOVE    #(CLK2+$030000+000+0000+SEH2+0000+0000+000+SFG1+0000+0000+0000),Y0	;42
;	MOVE    #(CLK2+$030000+000+000+000+000+H2L+H2R+000+000),Y0
	MOVE    Y0,Y:CCLK_1
; 42
;        MOVE    #$00F063,Y0             ; Channel 3 only
        MOVE    #$00F0C3,Y0             ; Channel 3 only
        MOVE    Y0,Y:SXMIT_VP           ; Put into VIDEO_PROCESS for arbitrary binning
        MOVE    Y0,Y:SXMIT_EH_1 ; and into all the hard-coded binned cases in high Y
        MOVE    Y0,Y:SXMIT_EH_2
        MOVE    Y0,Y:SXMIT_EH_3
        MOVE    Y0,Y:SXMIT_EH_4
...
ALIAS_B
	...
	; serials for general binning
	MOVE	#SERIAL_SKIP_FG,Y0
	MOVE	Y0,Y:SERIAL_SKIP
	MOVE    #INITIAL_CLOCK_FG,Y0
	MOVE    Y0,Y:INITIAL_CLOCK
	MOVE    #SERIAL_CLOCK_FG,Y0
	MOVE    Y0,Y:SERIAL_CLOCK
	; 42
        MOVE    #(CLK2+$030000+000+SEH1+0000+0000+0000+000+0000+SFG2+0000+0000),Y0
;	MOVE    #(CLK2+$030000+000+000+H1L+H1R+000+000+000+000),Y0
	MOVE    Y0,Y:CCLK_1
;        MOVE    #$00F042,Y0             ; Channel 2 only
        MOVE    #$00F082,Y0             ; Channel 2 only
        MOVE    Y0,Y:SXMIT_VP           ; Put into VIDEO_PROCESS for arbitrary binning
        MOVE    Y0,Y:SXMIT_FG_1 	; and into all the hard-coded binned cases in high Y
        MOVE    Y0,Y:SXMIT_FG_2
        MOVE    Y0,Y:SXMIT_FG_3
        MOVE    Y0,Y:SXMIT_FG_4
...
CMP_C	MOVE	#'__C',A		;  C Amplifier = e2v E amp = Leach channel #0
	...
	MOVE	X0,Y:<AMPVAL		; save the amp value
        ...
        MOVE    #AB_DOWN,Y0
        MOVE    Y0,Y:S_PAR_CLK           ; storage area parallels
        MOVE    #ABCD_CLEAR_DOWN,Y0
        MOVE    Y0,Y:IS_PAR_CLR          ; Clear full CCD
        MOVE    #AB_CLEAR_DOWN,Y0
        MOVE    Y0,Y:S_PAR_CLR           ; Clear storage area only
	...
	MOVE    Y0,Y:SERIAL_CLOCK
	; 42
        MOVE    #(CLK2+$030000+000+0000+SEH2+0000+0000+000+SFG1+0000+0000+0000),Y0
;	MOVE    #(CLK2+$030000+000+000+H1L+H1R+000+000+000+000),Y0
	MOVE    Y0,Y:CCLK_1
;        MOVE    #$00F000,Y0             ; Channel 0 only
        MOVE    #$00F000,Y0             ; Channel 0 only
        MOVE    Y0,Y:SXMIT_VP           ; Put into VIDEO_PROCESS for arbitrary binning
        MOVE    Y0,Y:SXMIT_EH_1 ; and into all the hard-coded binned cases in high Y
        MOVE    Y0,Y:SXMIT_EH_2
...
;	MOVE    #(CLK2+$030000+000+000+H1L+H1R+000+000+000+000),Y0
	MOVE    Y0,Y:CCLK_1
;        MOVE    #$00F021,Y0             ; Channel 1 only
        MOVE    #$00F041,Y0             ; Channel 1 only
        MOVE    Y0,Y:SXMIT_VP           ; Put into VIDEO_PROCESS for arbitrary binning
        MOVE    Y0,Y:SXMIT_FG_1 	; and into all the hard-coded binned cases in high Y
        MOVE    Y0,Y:SXMIT_FG_2
        MOVE    Y0,Y:SXMIT_FG_3
...
;	MOVE    #(CLK2+$030000+000+000+000+H1R+H2L+000+000+000),Y0
        MOVE    #(CLK2+$030000+000+0000+SEH2+0000+0000+000+0000+SFG2+0000+0000),Y0

	MOVE    Y0,Y:CCLK_1
; MOVE    #$00F062,Y0             ; Channels 2-3
        MOVE    #$00F0C2,Y0             ; Channels 2-3
	MOVE    Y0,Y:SXMIT_VP		; Put into VIDEO_PROCESS for arbitrary binning
        MOVE    Y0,Y:SXMIT_SPLIT_1      ; and into all the hard-coded binned cases in high Y
        MOVE    Y0,Y:SXMIT_SPLIT_2
        MOVE    Y0,Y:SXMIT_SPLIT_3
        MOVE    Y0,Y:SXMIT_SPLIT_4
...
;        MOVE    #$00F020,Y0             ; Channels 0-1
        MOVE    #$00F040,Y0             ; Channels 0-1
	MOVE    Y0,Y:SXMIT_VP		; Put into VIDEO_PROCESS for arbitrary binning
        MOVE    Y0,Y:SXMIT_SPLIT_1      ; and into all the hard-coded binned cases in high Y
        MOVE    Y0,Y:SXMIT_SPLIT_2
        MOVE    Y0,Y:SXMIT_SPLIT_3
...
;	MOVE    #(CLK2+$030000+000+000+000+H1R+H2L+000+000+000),Y0
        ...
        ;MOVE    #$00F041,Y0             ; Channels 1-2
        MOVE    #$00F081,Y0             ; Channels 1-2
	MOVE    Y0,Y:SXMIT_VP		; Put into VIDEO_PROCESS for arbitrary binning
        MOVE    Y0,Y:SXMIT_FG_1      ; and into all the hard-coded binned cases in high Y
        MOVE    Y0,Y:SXMIT_FG_2
        MOVE    Y0,Y:SXMIT_FG_3
...
CMP_ALL	MOVE	#'ALL',A		;  ABCD Amplifier = e2v EFGH amp = Leach channels 0-3
	...
	MOVE	X0,Y:<AMPVAL		; save the amp value
        ...
        MOVE    Y0,Y:IS_PAR_CLK          ; ganged parallels
        MOVE    #CD_UP,Y0
        MOVE    Y0,Y:S_PAR_CLK           ; storage area parallels; NA for split parallels
        MOVE    #ABCD_CLEAR_SPLIT,Y0
        MOVE    Y0,Y:IS_PAR_CLR          ; Clear full CCD
        MOVE    #CD_CLEAR_UP,Y0
        MOVE    Y0,Y:S_PAR_CLR           ; Clear storage area only; NA for split parallels
...
;	MOVE    #(CLK2+$030000+000+000+000+H1R+H2L+000+000+000),Y0
	...
	MOVE    Y0,Y:CCLK_1
;        MOVE    #$00F060,Y0             ; All four channels
        MOVE    #$00F0C0,Y0             ; All four channels
	MOVE    Y0,Y:SXMIT_VP		; Put into VIDEO_PROCESS for arbitrary binning
        MOVE    Y0,Y:SXMIT_SPLIT_1      ; and into all the hard-coded binned cases in high Y
        MOVE    Y0,Y:SXMIT_SPLIT_2
        MOVE    Y0,Y:SXMIT_SPLIT_3
...
; Disabled temporarily for LMI as well.
SET_TRIGGER
...
; Trigger support disabled temporarily for LMI as well.
WAIT_UNTIL_TRIGGER
...
; Note: LMI uses both storage and img area for singles for maximum frame.
