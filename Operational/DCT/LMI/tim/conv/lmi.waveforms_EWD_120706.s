...
; as per Confluence July 1 2012 end commentary
...
SI_DELAY        EQU     $A70000 ; 25 us parallel overlap for big CCD.
        ENDIF
...
R_DELAY         EQU     $060000 ; Fast serial register transfer delay
        ENDIF
...
RG_HI	EQU	+12.0	; Reset
RG_LO	EQU	+1.0	;
R_HI	EQU	+10.0	; Serials
R_LO	EQU	+1.0	;
SW_HI	EQU	+10.0	; Summing well, mode 1
SW_LO	EQU	+1.0	;
; SW_HI	EQU	+2.0	; Summing well, mode 2
; SW_LO	EQU	+2.0	;
SI_HI	EQU	+10.0	; Parallels
SI_LO	EQU      0.0	;
TG_HI	EQU	+10.0	; Transfer Gate
TG_LO	EQU	 0.0	;
DG_HI	EQU	+12.0	; Dump Gate
DG_LO	EQU	 0.0	;

; DC Bias voltages in volts
VOD	EQU	30.0	; Output Drain Left.
VRD	EQU	18.0	; Reset Drain Left 
VOG	EQU	 2.0	; Output Gate, mode 1
; VOG	EQU	+18.0	; Output Gate, mode 2
VDD	EQU	+29.0	; Dump Drain
...
DACS	DC	END_DACS-DACS-1
	...
	DC	CLKV+$000100+@CVI((R_HI+Vmax)/(2*Vmax)*255)	; Pin #1, S1 EH
	DC	CLKV+$000200+@CVI((R_LO+Vmax)/(2*Vmax)*255)
	DC	CLKV+$000400+@CVI((R_HI+Vmax)/(2*Vmax)*255)	; Pin #2, S2 EH
	DC	CLKV+$000800+@CVI((R_LO+Vmax)/(2*Vmax)*255)
	DC	CLKV+$002000+@CVI((R_HI+Vmax)/(2*Vmax)*255)	; Pin #3, S3 EF
	DC	CLKV+$004000+@CVI((R_LO+Vmax)/(2*Vmax)*255)
	DC	CLKV+$008000+@CVI((R_HI+Vmax)/(2*Vmax)*255)	; Pin #4, S1 FG
	DC	CLKV+$010000+@CVI((R_LO+Vmax)/(2*Vmax)*255)
	DC	CLKV+$020100+@CVI((R_HI+Vmax)/(2*Vmax)*255)	; Pin #5, S2 FG
	DC	CLKV+$020200+@CVI((R_LO+Vmax)/(2*Vmax)*255)
	DC	CLKV+$020400+@CVI((R_HI+Vmax)/(2*Vmax)*255)	; Pin #6, S3 GH
	DC	CLKV+$020800+@CVI((R_LO+Vmax)/(2*Vmax)*255)
	DC	CLKV+$022000+@CVI((SW_HI+Vmax)/(2*Vmax)*255)	; Pin #7, SW EH
	DC	CLKV+$024000+@CVI((SW_LO+Vmax)/(2*Vmax)*255)
	DC	CLKV+$028000+@CVI((SW_HI+Vmax)/(2*Vmax)*255)	; Pin #8, SW FG
	DC	CLKV+$030000+@CVI((SW_LO+Vmax)/(2*Vmax)*255)
	DC	CLKV+$040100+@CVI((RG_HI+Vmax)/(2*Vmax)*255)	; Pin #9, RG EH outputs
	DC	CLKV+$040200+@CVI((RG_LO+Vmax)/(2*Vmax)*255)
	DC	CLKV+$040400+@CVI((RG_HI+Vmax)/(2*Vmax)*255)	; Pin #10, RG FG outputs
	DC	CLKV+$040800+@CVI((RG_LO+Vmax)/(2*Vmax)*255)
	...
	DC	CLKV+$060100+@CVI((SI_HI+Vmax)/(2*Vmax)*255)	; Pin #13, AB1
	DC	CLKV+$060200+@CVI((SI_LO+Vmax)/(2*Vmax)*255)
	DC	CLKV+$060400+@CVI((SI_HI+Vmax)/(2*Vmax)*255)	; Pin #14, AB2
	DC	CLKV+$060800+@CVI((SI_LO+Vmax)/(2*Vmax)*255)
	DC	CLKV+$062000+@CVI((SI_HI+Vmax)/(2*Vmax)*255)	; Pin #15, AB3
	DC	CLKV+$064000+@CVI((SI_LO+Vmax)/(2*Vmax)*255)
	DC	CLKV+$068000+@CVI((SI_HI+Vmax)/(2*Vmax)*255)	; Pin #16, AB4
	DC	CLKV+$070000+@CVI((SI_LO+Vmax)/(2*Vmax)*255)
	DC	CLKV+$080100+@CVI((SI_HI+Vmax)/(2*Vmax)*255)	; Pin #17, CD1
	DC	CLKV+$080200+@CVI((SI_LO+Vmax)/(2*Vmax)*255)
	DC	CLKV+$080400+@CVI((SI_HI+Vmax)/(2*Vmax)*255)	; Pin #18, CD2
	DC	CLKV+$080800+@CVI((SI_LO+Vmax)/(2*Vmax)*255)
	DC	CLKV+$082000+@CVI((SI_HI+Vmax)/(2*Vmax)*255)	; Pin #19, CD3
	DC	CLKV+$084000+@CVI((SI_LO+Vmax)/(2*Vmax)*255)
	DC	CLKV+$088000+@CVI((SI_HI+Vmax)/(2*Vmax)*255)	; Pin #33, CD4
	DC	CLKV+$090000+@CVI((SI_LO+Vmax)/(2*Vmax)*255)
	DC	CLKV+$0A0100+@CVI((TG_HI+Vmax)/(2*Vmax)*255)	; Pin #34, TGA
	DC	CLKV+$0A0200+@CVI((TG_LO+Vmax)/(2*Vmax)*255)
	DC	CLKV+$0A0400+@CVI((TG_HI+Vmax)/(2*Vmax)*255)	; Pin #35, TGD
	DC	CLKV+$0A0800+@CVI((TG_LO+Vmax)/(2*Vmax)*255)
	DC	CLKV+$0A2000+@CVI((DG_HI+Vmax)/(2*Vmax)*255)	; Pin #36, DGA
	DC	CLKV+$0A4000+@CVI((DG_LO+Vmax)/(2*Vmax)*255)
	DC	CLKV+$0A8000+@CVI((DG_HI+Vmax)/(2*Vmax)*255)	; Pin #37, DGD
	DC	CLKV+$0B0000+@CVI((DG_LO+Vmax)/(2*Vmax)*255)
