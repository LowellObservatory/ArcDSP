...
;  Update the DACs
; IF A is non-zero an error was found in the DAC table
SET_DAC	MOVE	Y:(R0)+,X0		; Get the number of table entries
;  WE SHOULD VALIDATE X0 here (between 1 and something reasonable)
	MOVE	#MAXDACTBL,A
        CMP	X0,A
	JLT	FAIL
        CLR	A
...
; Ramp up the high +36 volt power line and then delay
	...
	MOVE	A,P:RSTWDT 
WT_PON2
; WE SHOULD set a status bit for INV levels
	BCLR	#ST_DDCLK,X:<STATUS
...
;       R0 passed from caller for the bias levels needed
	JSR	<SET_DAC
	TST	A
	JEQ	DACDONE
