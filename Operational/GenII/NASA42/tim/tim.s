...
; tim.s includes timmisc.s and timCCDmisc.s at the
; beginning and the waveforms file at the end in Y memory.
...
CC	EQU	CCDVIDREV3B+TIMREV4+UTILREV3+SUBARRAY+SPLIT_SERIAL
...
; Check for program overflow
        IF	@CVS(N,*)-$200>MISC_LEN
        WARN    'Miscellaneous P: programs are too large!' ; Make sure program
...
; Do the frame transfer if STORAGE==0, else skip it.
	MOVE	Y:<S_SIZE,X1
	JSR	ISHIFT		; Do the frame transfer; only Basic Occ does this
...
; CLR_SR MOVE	#(END_SERIAL_SKIP_SPLIT-SERIAL_SKIP_SPLIT-1),M1 ; Modularity
...
CONT_RD	DO      Y:<NPBIN,LPR_I          ; Transfer # of rows, with binning
	...
	MOVE    Y:<S_PAR_CLK,R0		; if STORAGE == 0 store clocks; Basic occ. only
...
IMAGE_MODE      DC      0
ISTATUS			DC		0
DSP_VERS	DC	VERSION ; code version must stay in loc,n 32!!
DSP_FLAV	DC	FLAVOR ;  type of dsp support must stay in loc,n 33!!
...
; They are locations 0x34, 35, and 36
...
NSR     DC      1072   	 	; Number Serial Read, prescan + image + bias
NPR     DC      1030	     	; Number Parallel Read
NS_CLR	DC      4146 		; To clear serial register, twice, w/ split serials
NPCLR   DC      4112    	; To clear parallel register, twice, w/ split parallels
NSBIN   DC      1       	; Serial binning parameter
NPBIN   DC      1       	; Parallel binning parameter
NROWS	DC	4112		; Number of physical rows in CCD
NCOLS	DC	4146		; Number of physical columns in CCD = 4096+50
...
S_SIZE		DC	2056	; Number of rows in the Storage Array; A+B or C+D
...
; Include the waveform table
	INCLUDE "42cam.waveforms.s" ; Readout and clocking waveforms
	...
	ENDSEC				; End of section TIMEEV57
