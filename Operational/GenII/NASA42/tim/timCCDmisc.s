...
OSHUT	BSET    #ST_SHUT,X:<STATUS 	; Set status bit to mean shutter open
	BSET	#SHUTTER,X:<LATCH	; Set hardware shutter bit to open
...
CSHUT	BCLR    #ST_SHUT,X:<STATUS 	; Clear status to mean shutter closed
	BCLR	#SHUTTER,X:<LATCH	; Clear hardware shutter bit to close
	MOVEP	X:LATCH,Y:WRLATCH	; Write it to the hardware
	MOVE    Y:<SH_DEL,A
        TST     A
        JLE     <S_DEL0
...
SET_SUBARRAY_POSITIONS
	...
	MOVE	#>21,X0
	CMP	X0,A
	JGT	<ERROR		; Error if number of boxes > 8
	MOVE	#READ_TABLE,X0
	ADD	X0,A
	MOVE	A1,R7
	MOVE	X:(R4)+,X0
...
; This is fairly complex.  The first value sent is NSR*IFRAMES (NAXIS1*NAXIS3).
; The second value is NPR (NAXIS2), but if NBOXES > 0, it is NPR*NBOXES
; The catch is that for a 4K CCD we use a little over 12 bits just in the 
; basic full frame dimension so we can't use NSR*NPR like we did with HIPO.
; I think we really only have 23 bits per number sent to the PCI card, so we
; can only go up to a little less than 11 bits (2048) IFRAMES.  There is probably a better way
; to factor this out, but we'll do this for now.
; modified to use  NAXIS2*NAXIS3 ,  NBOXES*NAXIS1 to fix strips problem,
; ed and pc, 11 Sep 2009
PCI_READ_IMAGE
	...
	ASR	B		; Correct for multiplication left shift
	MOVE	B0,X0	
	JSR	<XMT_FO		; Send NSR*IFRAMES to PCI card
