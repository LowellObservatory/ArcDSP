...
The following commands are supported in this "timmisc.asm" file
...
;  ** Place this file after the custom timmisc.asm file so it continues 
...
L_SEX0	MOVE	#L_SEX1,R7
	JMP	<EXPOSE			; Delay for specified exposure time
L_SEX1
	JCLR	#SHUT,X:STATUS,S_DEL0	; Close the shutter if necessary
...
S_DEL1	NOP
S_DEL0	NOP
	JMP	<RDCCD			; Finally, read out the CCD
...
PCI_READ_IMAGE
	...
	MOVE	#'RDA',X0
	JSR	<XMT_FO
	MOVE	Y:NSR,X0		; Number of columns to read
	JSR	<XMT_FO
	MOVE	Y:NPR,X0		; Number of rows to read		
	JSR	<XMT_FO
	RTS
