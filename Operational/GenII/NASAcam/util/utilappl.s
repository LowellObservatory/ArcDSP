...
; For initialization, depend on assembler setting to zero.
	...
	SUB	X,A
	JLT	<C_TEMP		; If T_COUNT .GE. 1024, find avg, update
	...
	MOVE	A0,Y:TH_SUM
	MOVE    A0,Y:TH_COUNT
; This is section handles the CCD temperature. It reads the temp then
...
C_TEMP
	...
	MOVE	Y:ONE_K,X0	; Get number to average
	SUB	X,A
	JLT	<SKIP_T		; If T_COUNT .GE. 1024, find avg, update
	...
	MOVE	A0,Y:<T_SUM
	MOVE	A0,Y:<T_COUNT
	MOVE    Y:<T_CCDT,A	; Get target CCD temperature
	SUB	X0,A
...
PWR_DLY	DO	#4000,L_DLY
	NOP			
L_DLY
	BCLR    #SHUTTER,X:PBD  ; Set Port B bit #3 to clos shutter
        BSET    #ST_SH,X:<STATUS ; Set status to mean shutter closed
	RTS
...
EXT_TMP EQU	AD_IN+7	; Address of the External temperature in A/D table
